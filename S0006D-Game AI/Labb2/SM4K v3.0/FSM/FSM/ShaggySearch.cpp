...
bool ShaggySearch::Search()
{
	...
	float startX = graph.getNode(startIndex).getPos().x;
	float startY = graph.getNode(startIndex).getPos().y;
	float goalX = graph.getNode(goalIndex).getPos().x;
	float goalY = graph.getNode(goalIndex).getPos().y;
